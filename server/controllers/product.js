...
const _productServiceInstance = new ProductService();
...
async function product_Upload( req, res ){
    var joResult = {};
    var oAuthResult = await _oAuthServiceInstance.verifyToken( req.headers['x-token'], req.headers['x-method'] );

    if( oAuthResult.status_code == "00" ){
        if( oAuthResult.token_data.status_code == "00" ){
            await _productServiceInstance.uploadFromExcel(req, res);
...
async function product_GetById( req, res ){
    var joResult;
    var errors = null;
    ...
    if( oAuthResult.status_code == "00" ){
        if( oAuthResult.token_data.status_code == "00" ){
            ...
            if( errors.length != 0 ){
                ...
                joResult = await _productServiceInstance.getById(req.params);
                joResult.token_data = oAuthResult.token_data;
                joResult = JSON.stringify(joResult);
            }
        }else{
...
async function product_DropDown( req, res ){
    var joResult;
    var errors = null;
    ...
    if( oAuthResult.status_code == "00" ){
        if( oAuthResult.token_data.status_code == "00" ){
            ...
            if( xError.length != 0 ){
                ...
                joResult = await _productServiceInstance.dropDownList(req.query);
                joResult.token_data = oAuthResult.token_data;
                joResult = JSON.stringify(joResult);
            }
        }else{
...
async function product_Save(req, res){
    var joResult;
    var errors = null;
